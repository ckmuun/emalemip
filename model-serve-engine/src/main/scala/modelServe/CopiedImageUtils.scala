...
object CopiedImageUtils {
  ...
  /*Note by cornelius koller:
    I took this code from databricks spark-dl lib and modified the accessors to public
    in order to make the functionality available outside the lib.
    However, these changes are insignificant and I do not hold any rights to this code.
   */
   ...
   */
  def spImageToBufferedImage(rowImage: Row): BufferedImage = {
   ...
   */
  def spImageFromBufferedImage(image: BufferedImage, origin: String = null): Row = {
  ...
  def spImageSchemaImplFromBufferedImage(image: BufferedImage, origin: String = null): ImageSchemaImpl = {
    val nChannels = image.getColorModel.getNumComponents
    val height = image.getHeight
    val width = image.getWidth
    val isGray = image.getColorModel.getColorSpace.getType == ColorSpace.TYPE_GRAY
    ...
    ImageSchemaImpl(origin, height, width, nChannels, getOCVType(image), decoded)
  }
